...
#pragma omp parallel for private(k)
        ...
        if ((TNBFlag > 0) && (mu > 1.6e-24) && (mu < 1.7e-24)) {
          TNB_DD(f[i], f_out[i], rank, TNBFlag, 1.0, mu, n[i], T[i]);
        }
      } else { // ij case
        if (!((n[i] < EPS_COLL) || (n[j] < EPS_COLL))) {
  ...
  if (TNBFlag > 0) {
    for (sp = 0; sp < nspec; sp++) {
      ...
      if ((mu > 1.6e-24) && (mu < 1.7e-24))
        TNB_DD(f[sp], f_out[sp], rank, TNBFlag, dt, 0.5 * m[sp], n_linear[sp],
               T_linear[sp]);
      ...
      for (sp2 = sp; sp2 < nspec; sp2++) {
        ...
        if ((mu > 1.8e-24) && (mu < 2.0e-24)) {
          TNB_DT(f[sp], f[sp2], f_out[sp], f_out[sp2], rank, TNBFlag, dt, mu,
                 n_linear[sp], n_linear[sp2], T_linear[sp], T_linear[sp2]);
        }
      }
    }
  }
